...
async def stand_pose(kos: KOS) -> None:
    """Execute the waving sequence."""
    # Motion parameters
    V_MAX = 100.0  # deg/s - reduced from 50.0 for smoother motion
    ACCEL = 150.0  # deg/s² - reduced from 80.0 for smoother acceleration
    dt = 1/50.0  # seconds
    
    # Configure right arm actuators with softer gains
    for actuator_id in actuator_ids:
        if actuator_id == 44 or actuator_id == 34:
            kp = 100.0
        else:
            kp = 50.0
        await kos.actuator.configure_actuator(
            actuator_id=actuator_id,
            kp=kp,  # reduced from 200.0
            kd=5.0,    # increased from 5.0 for better damping
            max_torque=50,
            torque_enabled=True
        )

    # await move_to_target(kos, 0.0, -10.0, ACCEL, V_MAX, dt, 41)
    # await move_to_target(kos, 0.0, 10.0, ACCEL, V_MAX, dt, 31)

    # await move_to_target(kos, -10.0, 0.0, ACCEL, V_MAX, dt, 41)
    # await move_to_target(kos, 10.0, 0.0, ACCEL, V_MAX, dt, 31)

    await asyncio.gather(
        move_to_target(kos, 0.0, 10.0, ACCEL, V_MAX, dt, 41),
        move_to_target(kos, 0.0, 0.0, ACCEL, V_MAX, dt, 42),
        move_to_target(kos, 0.0, -10.0, ACCEL, V_MAX, dt, 43),
        move_to_target(kos, 0.0, -20.0, ACCEL, V_MAX, dt, 44),
        move_to_target(kos, 0.0, 30.0, ACCEL, V_MAX, dt, 45),

        move_to_target(kos, 0.0, 20.0, ACCEL, V_MAX, dt, 31),
        move_to_target(kos, 0.0, 0.0, ACCEL, V_MAX, dt, 32),
        move_to_target(kos, 0.0, 10.0, ACCEL, V_MAX, dt, 33),
        move_to_target(kos, 0.0, 25.0, ACCEL, V_MAX, dt, 34),
        move_to_target(kos, 0.0, -10.0, ACCEL, V_MAX, dt, 35),
    )
    # await asyncio.gather(
    #     move_to_target(kos, -10.0, 0.0, ACCEL, V_MAX, dt, 41),
    #     move_to_target(kos, 10.0, 0.0, ACCEL, V_MAX, dt, 31),
    # )
