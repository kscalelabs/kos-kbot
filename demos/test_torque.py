...
async def wave_sequence(kos: KOS) -> None:
    ...
    # Motion parameters
    V_MAX = 40.0  # deg/s - reduced from 50.0 for smoother motion
    ACCEL = 60.0  # deg/s² - reduced from 80.0 for smoother acceleration
    dt = 1/40.0  # seconds
    ...
    await move_to_target(kos, 0.0, -80.0, ACCEL, V_MAX, dt, 21)  # shoulder pitch up
    await asyncio.sleep(0.5)
    
    # I want to move the wrist  Actuator(15, 17, 20.0, 2.0, 17.0),  # left_wrist_02
    # await move_to_target(kos, 0.0, -30.0, ACCEL, V_MAX, dt, 25)
    # await asyncio.sleep(0.5)
    # i want the elbow to move to 30 degrees
    # await move_to_target(kos, 0.0, 60.0, ACCEL, V_MAX, dt, 24)
    # # i want the wrist to move to 30 degrees
    # await move_to_target(kos, 0.0, 30.0, ACCEL, V_MAX, dt, 25)
    # await move_to_target(kos, 0.0, 100.0, ACCEL, V_MAX, dt, [51, 52, 53, 54, 55])
    # # await move_to_target(kos, 0.0, 100.0, ACCEL, V_MAX, dt, 52)
    # # await move_to_target(kos, 0.0, 100.0, ACCEL, V_MAX, dt, 53)
    # # await move_to_target(kos, 0.0, 100.0, ACCEL, V_MAX, dt, 54)
    # # await move_to_target(kos, 0.0, 100.0, ACCEL, V_MAX, dt, 55)
    

    # for _ in range(3):
    #     await move_to_target(kos, 0.0, 30.0, ACCEL, V_MAX, dt, 23)
    #     await asyncio.sleep(0.2)  # Short pause at each end
    #     await move_to_target(kos, 30.0, -30.0, ACCEL, V_MAX, dt, 23)
    #     await asyncio.sleep(0.2)
    #     await move_to_target(kos, -30.0, 0.0, ACCEL, V_MAX, dt, 23)
    #     await asyncio.sleep(0.2)
    # await move_to_target(kos, 100.0, 0.0, ACCEL, V_MAX, dt, [51, 52, 53, 54, 55])

    # await move_to_target(kos, 0.0, 60.0, ACCEL, V_MAX, dt, 24)
    # await move_to_target(kos, 0.0, 30.0, ACCEL, V_MAX, dt, 25)

    # await move_to_target(kos, -80.0, 0.0, ACCEL, V_MAX, dt, 21)
