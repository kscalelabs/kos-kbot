...
async def wave_sequence(kos: KOS) -> None:
    ...
    right_arm_ids = [11, 12, 13, 14, 15]  # shoulder pitch, roll, yaw, elbow, wrist
    ...
    await move_hands(kos, left_hand_ids, 0)
    # Wave sequence with pauses between movements
    await asyncio.gather(
        move_to_target(kos, 5.0, 70.0, ACCEL, V_MAX, dt, 12),  # shoulder roll up
        move_to_target(kos, 0.0, 60.0, ACCEL, V_MAX, dt, 11),  # shoulder pitch up
        move_hands(kos, left_hand_ids, 1),
        move_to_target(kos, 0.0, 20.0, ACCEL, V_MAX, dt, 13),
        move_to_target(kos, 0.0, -125.0, ACCEL, V_MAX, dt, 14),
        
    )
    await move_hands(kos, [61, 62, 65, 66], 0)
    await asyncio.sleep(3.5)
    input("press enter to return" )
    await move_hands(kos, left_hand_ids, 1),
    await asyncio.gather(
        move_hands(kos, [61, 62, 65, 66], 1),
        move_to_target(kos, -125.0, 0.0, ACCEL, V_MAX, dt, 14),
        move_to_target(kos, 20.0, 0.0, ACCEL, V_MAX, dt, 13),
        move_to_target(kos, 60.0, 0.0, ACCEL, V_MAX, dt, 11),
        move_to_target(kos, 70.0, 5.0, ACCEL, V_MAX, dt, 12)
    )
