...
async def move_hands(kos: KOS, hand_ids: list[int], position: int) -> None:
    ...
    target = 50.0 if position == 1 else 0.0
...
async def wave_sequence(kos: KOS) -> None:
    ...
    await move_to_target(kos, 0.0, -20.0, ACCEL, V_MAX, dt, 21)
    await move_to_target(kos, 0.0, 50.0, ACCEL, V_MAX, dt, 24)
    await move_hands(kos, right_hand_ids, 1)
    # Correct usage - this runs all moves in parallel
    # await asyncio.gather(
    #     move_to_target(kos, 0.0, -70.0, ACCEL, V_MAX, dt, 22),  # shoulder roll up
    ...
    #     move_to_target(kos, -70.0, 0.0, ACCEL, V_MAX, dt, 22)
    # )
